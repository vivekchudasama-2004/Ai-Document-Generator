...
def test_auto_tiers_and_reasons():
    ...
    assert simple == "meta/llama-3.1-8b-instruct" and reasons
    ...
    assert medium == "meta/llama-3.1-70b-instruct"
    ...
    assert flagship == "meta/llama-3.1-405b-instruct"
    short_human, _ = auto_select_model(role="humanize", text="Small section here.")
    assert short_human == "meta/llama-3.1-8b-instruct"
    long_human, _ = auto_select_model(role="humanize", text="x" * 5000)
    assert long_human == "meta/llama-3.1-70b-instruct"
...
def test_resolve_auto_is_default():
    ...
    assert resolve_model("generate", "auto") == "meta/llama-3.1-8b-instruct"  # empty brief
    assert resolve_model("humanize", "auto", text="hi") == "meta/llama-3.1-8b-instruct"
...
def test_models_endpoints(client, auth_headers):
    ...
    put = client.post("/api/models/enabled", headers=auth_headers,
                      json={"model_id": "meta/llama-3.1-70b-instruct", "enabled": True})
    assert put.json() == {"model_id": "meta/llama-3.1-70b-instruct", "enabled": True}
    enabled = client.get("/api/models/enabled", headers=auth_headers).json()["items"]
    assert [m["id"] for m in enabled] == ["meta/llama-3.1-70b-instruct"]
    ...
    remove = client.post("/api/models/enabled", headers=auth_headers,
                         json={"model_id": "meta/llama-3.1-70b-instruct", "enabled": False})
